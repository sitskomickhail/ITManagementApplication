...
namespace ITManagementClient.ViewModels.Developer
{
    public class DeveloperControlViewModel : BaseViewModel, IPageViewModel
    {
        private Dictionary<string, IControlViewModel> _developerControls;
        public Dictionary<string, IControlViewModel> DeveloperControls => _developerControls ?? (_developerControls = new Dictionary<string, IControlViewModel>());
        ...
        public ICommand ShowMyProjectsCommand { get; set; }

        public DeveloperControlViewModel()
        {
            ...
            foreach (var viewModel in types)
            {
                ...
                if (viewModelInstance != null && viewModelInstance.PageDefinitions.Any(p => p == PageDefinition.Developer))
                {
                    DeveloperControls.Add(viewModel.Name, viewModelInstance);
            ...
            ShowMyProjectsCommand = new RelayCommand(ShowMyProjectsCommandExecute);
        }
        ...
        private void ShowMyProjectsCommandExecute(object obj)
        {
            Mediator.Notify(nameof(DeveloperProjectsListViewModel), nameof(DeveloperProjectsListViewModel));
        }
    }
}